...
# Load Model
model_For_Testing = pickle.load(open('Models/KNNModel3.pk1', 'rb'))

# Load data
trainingData = pandas.read_csv("Data for ML/CIC/CIC_DataFrame.csv")
target = trainingData['attack_cat'].values.astype(int)

'''trainingData = trainingData.drop(columns=trainingData.columns.difference(['attack_cat', 'swin', 'dwin', 'dmeansz', 'dsport', 'Flow Bytes/s', 'proto', 'Spkts', 'Dpkts', 'tcprtt']))'''
...
# Make explainer
background_sample = shap.sample(input_test, 100)
...
for i, class_shap_values in enumerate(shap_values):
    ...
    png_file_path = f"Explainer Charts/Summary/On Test/KNN Model 3/shap_plot_class{i}.png"
