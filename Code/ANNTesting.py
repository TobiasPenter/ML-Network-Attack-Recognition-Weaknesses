...
#Make explainer
background_sample = shap.sample(input_test, 1000)
...
for j in range (10):
    instance = random.randint(0, len(background_sample) - 1)

    for i, class_shap_values in enumerate(shap_values):  
        shap_instance = class_shap_values[instance]
        print(f"Generating plot for instance {instance}, class {i}")
        force_plot = shap.force_plot(explainer.expected_value[i], shap_instance, background_sample.iloc[instance].values, feature_names=background_sample.columns, show=False)
